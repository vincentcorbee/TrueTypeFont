...
const showGlyph = (
  ...
  const glyph =
    typeof code === 'string' ? font.getGlyphByChar(code) : font.getGlyphByUnicode(code)
...
inputFile.addEventListener('change', (e: any) => {
  ...
  if (file) {
    ...
    fr.addEventListener('loadend', () => {
      ...
      overview.innerHTML = `<div class='flex row'>
        <span class='label'>FontFamily</span><span>${font.fontFamily}</span>
      </div>
      ...
      font.readGlyphs((glyph: any, i: number) => {
        if (glyph) {
          ...
          details.innerHTML = `<div class='flex'><span class='label'>Unicode:</span> <span>${glyph.unicode}</span></div><div class='flex'><span class='label'>Name:</span><span>${glyph.name}</span></div>`
          details.className = 'details'
          icon.className = 'icon'
