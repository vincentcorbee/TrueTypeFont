export type TableName = 'post' | 'cmap' | 'name' | 'GSUB' | 'glyf' | 'head' | 'hhea' | 'hmtx' | 'loca' | 'maxp' | 'OS/2' | 'cvt' | 'fpgm' | 'hdmx' | 'kern' | 'prep';
export type RequiredTableName = Extract<TableName, 'cmap' | 'glyf' | 'head' | 'hhea' | 'hmtx' | 'loca' | 'maxp' | 'name' | 'post'>;
export type OptionalTableName = Extract<TableName, 'OS/2' | 'cvt' | 'pfgm' | 'hdmx' | 'kern' | 'prep' | 'GSUB'>;
...
export type platformSpecificEncoding = 'Roman' | 'Japanese' | 'Traditional Chinese' | 'Korean' | 'Arabic' | 'Hebrew' | 'Greek' | 'Symbol' | 'Unicode BMP' | 'Unicode full repertoire' | 'Unicode 1.0 semantics' | "Unicode 2.0 and onwards semantics, Unicode BMP only ('cmap' subtable formats 0, 4, 6)";
