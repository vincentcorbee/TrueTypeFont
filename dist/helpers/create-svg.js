...
const create_circle_1 = __importDefault(require("./create-circle"));
const draw_curves_1 = __importDefault(require("./draw-curves"));
const transform_1 = __importDefault(require("./transform"));
function createSVG(font, glyph, { fontSize = 64, useBB = true, showPoints = false, showLines = false }) {
    const createGlyph = (glyph = { points: [], endPtsOfContours: [0] }, height = 0, scale = 1) => {
        ...
        while (pointIndex < lengthPoints) {
            ...
            if (isFirstContourPoint) {
                ...
                if (showPoints) {
                    ...
                    refPoints.push((0, create_circle_1.default)(x, y, 'orange', scale));
                }
                ...
                if (point.onCurve) {
                    if (ctrlPoints.length)
                        d += (0, draw_curves_1.default)(ctrlPoints, point, height, scale);
                    else
                        d += ` L ${x} ${y}`;
                    path.setAttribute('d', d);
                    if (showPoints)
                        refPoints.push((0, create_circle_1.default)(x, y, 'blue', scale));
                }
                else {
                    ctrlPoints.push(point);
                    if (showPoints)
                        refPoints.push((0, create_circle_1.default)(x, y, 'red', scale));
                }
            }
            if (pointIndex === glyph.endPtsOfContours[contourNumber]) {
                ...
                if (!point.onCurve)
                    d += (0, draw_curves_1.default)(ctrlPoints, firstContourPoint, height, scale);
                path.setAttribute('d', d);
                docFrag.appendChild(path);
                if (showPoints) {
                    refPoints.push((0, create_circle_1.default)(point.x * scale, height - point.y * scale, point.onCurve ? 'black' : 'purple', scale));
                }
    ...
    if (showLines) {
        ...
        descenderLine.setAttribute('d', `M ${minX} ${viewBoxHeight + minY - 10 * viewBoxScale} H ${viewBoxWidth + minX}`);
        descenderLine.setAttribute('stroke-width', (10 * viewBoxScale).toString(10));
        descenderLine.setAttribute('stroke', 'grey');
        accendererLine.setAttribute('d', `M ${minX} ${minY + 10 * viewBoxScale} H ${viewBoxWidth + minX}`);
        ...
        xHeightLine.setAttribute('d', `M ${minX} ${viewBoxHeight - xHeight * viewBoxScale} H ${viewBoxWidth + minX}`);
        ...
        glyph.components.forEach(component => {
            ...
            if (componentGlyph) {
                componentGlyph.points =
                    componentGlyph.points?.map(point => (0, transform_1.default)([a, b, c, d, e, f], point)) ?? [];
