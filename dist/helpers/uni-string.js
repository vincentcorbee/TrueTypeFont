...
const uniString = (str) => {
    ...
    return {
        codePointAt: (index) => {
            const c = arr[index];
            return !c ? 0 : Array.isArray(c.hex) ? c.hex.length > 1 ? c.hex.map((v) => parseInt(v, 16)) : parseInt(c.hex[0], 16) : parseInt(c.hex, 16);
        },
        charAt: (index) => {
            const c = arr[index];
            if (!c) {
            ...
            const point = Array.isArray(c.hex) ? c.hex.map((v) => parseInt(v, 16)) : parseInt(c.hex, 16);
            let o = '';
            if (c.type === 'surrogate') {
                ...
                if (point.length > 1) {
                    o = point.map((p) => JSON.parse('"\\u' + `${toHex(p)}"`)).join('');
                }
                else {
...
exports.default = uniString;
//# sourceMappingURL=uni-string.js.map